...
User.prototype.cleanUp = function () {
  ...
  if (typeof this.data.samePassword !== "string") {
    this.data.samePassword == "";
  }
  ...
  if (this.data.password) {
    this.data.password = this.data.password.trim().toLowerCase();
  }
  if (this.data.samepassword) {
    this.data.samepassword = this.data.samepassword.trim().toLowerCase();
  }
...
User.prototype.validate = function () {
  return new Promise((resolve, reject) => {
    ...
    if (
      ...
      this.errors.push("Usernames can only conatin letters or numbers");
    }
    ...
    if (this.data.username.length > 30) {
      this.erros.push("Usernames cannot exceed 50 characters");
    }
    ...
    if (this.data.password != this.data.samepassword) {
      this.errors.push("Passwords must be matching");
    }
